...
title: "如何用 n8n 和 LLM 自动分类个人邮件，搭建稳定省时、可长期运行的智能收件箱自动化实践指南教程"
date: 2026-04-11
description: "这篇文章分享我已稳定运行数月的个人邮箱自动化方案，使用 n8n 串联 Gmail 与 GPT-5-nano，让 LLM 读取来信内容后自动判断该归档、稍后阅读还是优先回复，适合想减少手动分拣、提升处理效率、降低遗漏重要邮件风险，并建立更清晰、更可持续收件箱流程的个人用户参考方案。"
...
我早就受够了等 Gmail 自己判断哪些邮件对我真的重要。让 AI 接手我的个人邮件分类三个月之后，我已经很难想象再回到手动整理收件箱的日子。

这套系统简单到近乎粗暴：GPT-5-nano 会把每封邮件判定为三种处理方式之一。归档、阅读，或者回复。就这么简单。

整套流程都跑在 n8n 上。因为我用的是 OpenAI 最便宜、而且支持结构化输出的模型，所以成本几乎可以忽略不计。没错，现在 OpenAI 能看到我所有个人邮件了，但说实话，他们大概早就从我在 ChatGPT 里的对话里知道更多了。

这不是拿来兜售概念的纸面方案。我已经把它稳定地跑了三个月，每周都能实打实地省下好几个小时。

## 我为什么做这套邮件自动化系统

我收到的邮件实在太多了。新闻订阅、服务通知、真人发来的重要消息，全都挤在同一个收件箱里。Gmail 自带的分类经常漏掉一些再明显不过的规律，而我每天早上都得花 20 分钟，光是判断哪些邮件值得看。

真正让我下决心动手的，是我发现自己有 80% 的邮件其实连看都不看，直接就归档了。如果大多数邮件根本不需要我注意，那为什么还要由我亲自判断哪些该看？
...
每当 Gmail 收件箱里有新邮件进来，这个工作流就会触发。它会抓取邮件内容，发给 OpenAI 做分类，然后在 Gmail 里打上对应标签，把那些暂时不需要我处理的邮件直接归档。
...
> **说明**：下面只是邮件分类用的 LLM 提示词。如果你想看完整的 n8n 自动化工作流（包括所有节点和连接关系），请直接往下滚到“完整 n8n 工作流 JSON”那一节。
...
我用结构化输出，是为了确保 AI 始终返回合法类别。这样既不会出现解析错误，也能避免模型突然用奇怪格式作答的边缘情况。
...
5% 的误差完全可以接受。AI 分错的时候，我只要把邮件移到正确类别就行，整体还是比我手动一封封分拣快得多。

## 安全性怎么权衡（以及我为什么能接受）

是的，OpenAI 现在会处理我所有个人邮件。从隐私角度看，这当然谈不上理想，但我最后还是接受了，主要有几个原因。
...
## 为什么这比 Gmail 自带功能更适合我

Gmail 的自动分类是为所有人设计的，也就意味着它不可能真正贴合任何一个具体的人。我的系统学的是我自己的邮件模式和处理偏好。
