...
title: "如何用 n8n 和 LLM 自动分类个人邮件，建立归档、稍后阅读和需要回复三类个人收件箱自动化工作流"
date: 2026-04-11
slug: "ruhe-yong-n8n-he-llm-zidong-fenlei-youjian"
description: "这篇文章分享我已经稳定运行三个月的邮件自动分类流程：通过 n8n 连接 Gmail 与 GPT-5-nano，把每封新邮件自动分到归档、稍后阅读和需要回复三类，减少每天手动整理收件箱的时间，也让新闻订阅、服务通知和真正需要我答复的来信区分得更清楚，同时保持规则简单、成本很低、后续维护也不复杂。"
...
# 如何用 n8n 和 LLM 自动分类个人邮件

我早就受够了等 Gmail 自己判断哪些邮件对我真的重要。让 AI 接手我的个人邮件分类三个月后，我已经很难想象再回到手动整理收件箱的日子。

这套系统简单得近乎粗暴：GPT-5-nano 会把每封邮件判定为三类之一：归档、稍后阅读，或者需要回复。就这么简单。

整套流程都跑在 n8n 上。因为我用的是 OpenAI 最便宜、同时支持结构化输出的模型，所以成本几乎可以忽略。没错，现在 OpenAI 能看到我所有个人邮件了，但说实话，他们可能早就从我在 ChatGPT 里的对话里知道更多了。

这不是什么拿来讲概念的纸上方案。我已经把它稳定跑了三个月，每周都能实打实省下好几个小时。

## 我为什么做这套邮件自动化系统

我收到的邮件实在太多了。新闻订阅、服务通知、真人发来的重要消息，全都挤在同一个收件箱里。Gmail 自带的分类经常漏掉一些再明显不过的规律，而我每天早上都得花 20 分钟，光是判断哪些邮件值得点开。

真正让我下决心动手的，是我发现自己有 80% 的邮件连看都不看，直接就归档了。如果大多数邮件根本不需要我注意，那为什么还要由我亲自判断哪些该看？

> **更喜欢视频教程？** 我录了一段分步骤演示，把这套邮件自动化工作流从头到尾跑了一遍。[查看用 n8n 和 LLM 实现邮件自动化的教程](https://www.youtube.com/watch?v=xn1RyMlZudE)，可以直接看到完整的配置过程。
...
我的 AI 会把每一封邮件严格归到下面三类之一：

- **归档** - 我订阅了但基本不会看的新闻邮件、自动通知、促销邮件
- **稍后阅读** - 我想看，但不需要回信的内容
- **需要回复** - 需要我亲自答复的邮件

不需要复杂的文件夹结构，也不需要优先级体系。就这三类，已经能覆盖我全部的邮件处理场景。

## 如何搭建这个 n8n 工作流

下面就是把这一切自动跑起来的完整 n8n 工作流：

![n8n 邮件分类工作流](/articles/assets/n8n-email-workflow-full.webp)

每当 Gmail 收件箱里有新邮件进来，这个工作流就会触发。它会抓取邮件内容，发给 OpenAI 做分类，然后在 Gmail 里打上对应标签，把暂时不需要我处理的邮件直接归档。
...
2. **OpenAI Chat Model** - 负责判断邮件类别
3. **Gmail** - 添加标签并归档邮件
4. **IF** 条件节点 - 根据 AI 的判断把邮件分流到不同分支

真正的关键在 OpenAI 节点怎么配置。下面是我的设置方式：

![n8n 中 OpenAI LLM 节点的配置](/articles/assets/n8n-llm-node-config.webp)

## 让这套系统稳定工作的提示词

提示词是这套系统的核心。试了几十种写法之后，这一版给我的结果最稳定：

> **说明**：下面这段 prompt 我保留英文原文，方便你直接复制到 n8n 或 OpenAI 节点里使用。它只对应邮件分类逻辑；如果你想看完整的 n8n 自动化工作流（包括所有节点和连接关系），请直接往下滚到“完整 n8n 工作流 JSON”那一节。

<details>
<summary>点击展开完整英文提示词</summary>
...
我这里用结构化输出，是为了确保 AI 每次都返回合法类别。这样既不会出现解析错误，也能避免模型临时发挥，把返回格式搞乱。
...
5% 的误差完全在可接受范围内。AI 分错的时候，我只要把邮件移到正确类别就行，整体还是比手动一封封分拣快得多。

## 安全性上的取舍（以及为什么我能接受）

是的，OpenAI 现在会处理我所有个人邮件。从隐私角度看，这当然不算理想，但我最后还是接受了，主要有几个原因。

第一，我早就用密码管理器和不依赖邮件的 2FA 验证码，所以即便邮箱本身出问题，也不至于造成灾难。第二，OpenAI 通过我平时使用 ChatGPT，本来就已经知道不少关于我的事。第三，对我这种个人工作流来说，省下来的时间值得这点隐私上的取舍。
...
如果你已经熟悉 n8n，整个配置大概 30 分钟就够了。要是从零开始，可能需要一个小时左右。

## 为什么它比 Gmail 自带功能更好用

Gmail 的自动分类是为所有人设计的，也就意味着它很难真正贴合某一个具体的人。我的系统针对的是我自己的邮件模式和处理偏好。

而且我随时都能改逻辑。想加第四类？改提示词。需要对某些发件人做特殊处理？加一个条件节点。Gmail 的规则很僵硬，而这套系统可以跟着我的需求一起变。

## 最后总结

跑了三个月之后，这套邮件自动化系统已经成了我日常工作流里离不开的一部分。它并不完美，但和每周手动整理几百封邮件相比，已经省心太多了。

配置不复杂，持续成本很低，节省下来的时间也是真实存在的。如果你也像我之前一样被邮件淹没，这个方法值得试试看。

只要记住一点：先从最简单的版本开始，充分测试，不要自动化任何你没法轻松撤销的东西。
...
1. 复制上面的 JSON
2. 把它保存成名为 `email-ai-automation-personal.json` 的文件
3. 在 n8n 里进入 `Workflows → Import from JSON`
4. 选择你保存的文件，然后点击 `Import`
5. 配置 Gmail 和 OpenAI 凭据
...
如果你更喜欢视频学习，我也录了一个完整教程，把这套邮件自动化系统从搭建到测试完整走了一遍：

[![用 n8n 和 LLM 做邮件自动化的视频教程](https://img.youtube.com/vi/xn1RyMlZudE/maxresdefault.jpg)](https://www.youtube.com/watch?v=xn1RyMlZudE)

视频里会展示如何创建 n8n 工作流、配置 OpenAI 集成、连接 Gmail，以及测试整个自动化流程的每一步。你会看到各个节点的具体配置、LLM 提示词的实际效果，以及系统如何实时分类真实邮件。
