...
title: "如何用 n8n 和 LLM 自动分类个人邮件，建立归档、阅读和回复三种处理动作的完整收件箱工作流教程"
date: 2026-04-11
slug: "ruhe-yong-n8n-he-llm-zidong-fenlei-youjian"
description: "这篇文章分享我已经稳定运行三个月的邮件自动分类流程：通过 n8n 连接 Gmail 与 GPT-5-nano，把每封新邮件自动归到归档、阅读或回复三类，减少每天手动筛选收件箱的时间，也让新闻订阅、服务通知和真正需要我答复的来信被更清楚地区分开，同时保持规则简单、成本很低、后续维护也不复杂。"
...
这套系统简单得近乎粗暴：GPT-5-nano 会把每封邮件判定为三种处理方式之一：归档、阅读，或者回复。就这么简单。

整套流程都跑在 n8n 上。因为我用的是 OpenAI 最便宜、而且支持结构化输出的模型，所以成本几乎可以忽略。没错，现在 OpenAI 能看到我所有个人邮件了，但说实话，他们可能早就从我在 ChatGPT 里的对话里知道更多了。

这不是什么纸上谈兵的概念。我已经把它稳定跑了三个月，每周都能实打实省下好几个小时。

## 我为什么做这套邮件自动化系统

我收到的邮件实在太多了。新闻订阅、服务通知、真人发来的重要消息，全都挤在同一个收件箱里。Gmail 自带的分类经常漏掉一些再明显不过的规律，而我每天早上都得花 20 分钟，光是判断哪些邮件值得读。

真正让我下决心动手的，是我发现自己有 80% 的邮件连看都不看，直接就归档了。如果大多数邮件根本不需要我注意，那为什么还要由我亲自判断哪些该看？

> **更喜欢视频教程？** 我录了一段分步骤演示，把这套邮件自动化工作流从头到尾跑了一遍。[查看用 n8n 和 LLM 实现邮件自动化的教程](https://www.youtube.com/watch?v=xn1RyMlZudE)，可以直接看到完整配置过程。
...
我的 AI 会把每一封邮件严格归到三类之一：

- **归档** - 我订阅了但基本不看的新闻邮件、自动通知、促销邮件
- **阅读** - 我想看，但不需要回信的内容
- **回复** - 需要我亲自答复的邮件

不需要复杂的文件夹结构，也不需要优先级体系。就这三个动作，已经能覆盖我全部的邮件处理场景。
...
我这里用结构化输出，是为了确保 AI 始终返回合法类别。这样既不会出现解析错误，也能避免模型突然用奇怪格式作答。
...
5% 的误差完全可以接受。AI 分错的时候，我只要把邮件移到正确类别就行，整体还是比手动一封封分拣快得多。
...
## 为什么它比 Gmail 自带功能更适合我

Gmail 的自动分类是为所有人设计的，也就意味着它不可能真正贴合任何一个具体的人。我的系统针对的是我自己的邮件模式和处理偏好。

而且我随时都能改逻辑。想加第四个类别？改提示词。需要对某些发件人做特殊处理？加一个条件节点。Gmail 的规则很僵硬，而这套系统可以跟着我的需求一起变。
