---
title: "Codex 规则：用全局指令、个人 AGENTS.md 和 Mac 应用统一跨仓库长期开发规则与编码习惯"
date: 2026-03-21
description: "这篇文章介绍我如何使用 Codex 自定义指令、个人 AGENTS.md 和 Mac 应用，把长期有效的编码习惯、显式报错要求、文档写法与仓库协作规则带进不同项目，让应用与 CLI 默认共享同一套开发基线，减少每次进入新仓库时重复说明和重新校准的成本，同时保留真实文件带来的清晰层次。"
...
我认真开始使用 Codex，其实也就一个月左右，主要还是在 GPT-5.4 让我明显感觉它更好用之后。所以这不是一篇“经过五年实战检验的终极经验”文章。我的观察很简单：当我不再把 Codex 当成另一个花哨的提示词输入框，而是先给它一套稳定、长期有效的指令之后，它立刻就变得对我有用了。

OpenAI 同时提供 Codex CLI 和 Mac 应用。对我来说，它们背后的逻辑是一样的：长期生效的指令、`AGENTS.md`、仓库规则，以及一个启动时就已经带着我开发习惯的代理。我更偏爱 Mac 应用，因为日常用起来，它确实比再开一个终端窗口舒服得多。

Codex CLI 本身已经把这部分做得很好。Mac 应用则是在同一套 Codex 工作流上，给了我一个更顺手、更舒适的界面。我在意的还是同一套长期开发习惯：严格类型、尽量少改、显式报错、不要随手加兜底逻辑、把说明写进代码里的文档字符串，而不是散落在各处。我不想在每个新任务里都从头把这些重新教给 Codex。我希望这层基线从一开始就已经在那里。

实际设置入口就在 `Settings -> Personalization -> Custom instructions`。

更底层一点看，这些应用里的指令会映射到个人 `AGENTS.md`。这样很好。我既能得到更舒服的应用体验，也不会失去真实文件带来的清晰感。
...
这正是我想要的结构。我可以把应用当成主要界面来用，同时依然保留背后真实文件带来的清晰层次。
...
这是我希望 Codex 在看到项目级指令之前，就先自动带进任何仓库的一层基础规则。
...
它存在的意义，就是消除那些反复出现的摩擦：

- 我只想补一个小改动时，Codex 却擅自扩成大范围重构
- Codex 用含糊的说法掩盖自己其实并不确定
- Codex 出于“安全考虑”擅自加上兜底逻辑
- Codex 因为任务说明太窄而忽略仓库约定

这些规则先加载好之后，整个会话都会平静很多。

## 为什么我把全局规则放在仓库规则前面

Codex 对 `AGENTS.md` 的分层支持做得不错。OpenAI 文档里明确写了全局文件 `~/.codex/AGENTS.md`，然后随着工作目录越来越具体，再叠加仓库级和更深层目录里的文件。

这种分层当然有用，但第一层首先必须是我自己的规则。
...
如果把它们混在一起，最后只会得到重复、偏移，以及一个没人愿意维护的文件。

这也是 Codex 比我原本预期更好用的原因之一。它的指令层级是摆在明面上的。它不像一套藏起来的提示词小把戏，更像一个真正的系统。

## Mac 应用是主要使用界面，这一点很重要

我现在最喜欢的部分，是新的 Codex Mac 应用。

这并不是因为 CLI 不行。CLI 已经很好了。只是应用日常用起来更舒服。底下还是同一个 Codex，上面只是换成了一个更顺手的界面。

所以尽管 CLI 很重要，我也不想把这篇文章的重心放在 CLI 上。对我来说，应用才是使用这套系统更舒服的入口。

而真正让这个应用不只是“好看”，而是“靠谱”的地方，在于这些指令依然由 `AGENTS.md` 托底。应用文档说，编辑自定义指令会更新个人 `AGENTS.md`，这正是我想要的关系：

- 应用设置负责方便
- 文件里的指令负责长期稳定

这也让之后理解 CLI 的用法变得更简单，因为同一套基础指令会原样延续过去。

## 项目级 AGENTS.md 依然重要，只是不是主角

我不想让这篇文章变成一篇讲 `AGENTS.md` 嵌套层级的教程，虽然 Codex 完全支持那套做法。
...
如果我打开一个陌生仓库，而 Codex 表现很差，我希望能很快定位原因。通常答案应该是下面几种之一：
...
Mac 应用是我的主界面。CLI 不是什么次一级的兜底选择，它只是同一套 Codex 系统的另一种形态。

CLI 仍然重要，原因有几个：

- 它让基于文件的配置一目了然
- 它更容易精确检查某些行为，也更方便写脚本

我不想给 CLI 再发明一套独立的世界观。我想要的是同样的个人 `AGENTS.md`、同样的仓库说明，以及两边一致的边界。

这种连续性，是这个产品让我觉得整体很完整的重要原因。
...
打开 Codex 应用设置，按 `Cmd+,`，进入 `Personalization`，先把自定义指令写在那里。虽然我脑子里还是会把它理解成 `~/.codex/AGENTS.md`，但应用确实是我主要设置和查看的地方。
...
命令、架构、约束、测试预期、命名、危险区域。这些才属于仓库层。
...
到目前为止，最适合我的配置，恰恰也是最不花哨的那种：写好稳定的长期指令，把仓库说明和个人偏好分开，让代理一开始就站在与你工作方式一致的起点上。

和 Cursor 一样，和 Claude Code 也一样。产品不同，结论却差不多：当代理不再猜测你是谁时，整个会话就会顺很多。
