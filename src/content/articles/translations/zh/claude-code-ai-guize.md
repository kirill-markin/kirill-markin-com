...
在那之前，我总得反复说同样的话。用严格类型。不要加我没要求的 fallback。保持 diff 尽量小。别一兴奋就重写半个文件。Claude 通常也会听，但这些话我还是得一遍又一遍地重新说。

现在，在仓库本身进入对话之前，这层基础设定就已经在那里了。

## 我的 Claude Code 全局指令放在哪里

Anthropic 的 [Claude Code 文档](https://docs.anthropic.com/en/docs/claude-code/overview)把这件事分成了几层。`~/.claude/CLAUDE.md` 是用户级文件，`./CLAUDE.md` 或 `./.claude/CLAUDE.md` 是共享的项目层，`CLAUDE.local.md` 则用于你自己的项目备注，不该进 git 的内容就放这里。

这几乎就是我想要的结构：

1. `~/.claude/CLAUDE.md` 放我长期稳定的编码偏好
2. 项目里的 `CLAUDE.md` 写仓库特有的架构和命令
3. `CLAUDE.local.md` 放那些不该进 git 的个人项目笔记

我不想每打开一个仓库，都把同样的个人规则重新粘贴一遍。如果“不要静默 fallback”是全局偏好，那它就应该放在全局文件里。如果“跑测试前先执行这个奇怪的内部 bootstrap 命令”只和某个仓库有关，那它就应该放在项目文件里。
...
这段内容我故意写得很无聊。好的规则通常就是这样。我不是想提前描述所有边缘情况，只是想让 Claude Code 别再犯那些一再出现、完全可以预见的错误。
...
没有这层约束时，Claude 会用几种非常熟悉的方式慢慢跑偏。它会“以防万一”地加上 fallback。它会因为严格类型显得麻烦，就把类型放宽。它会给本来很简单的函数多包几层。它还会因为想显得配合，最后修错了问题，因为它在努力迁就，而不是努力精确。
...
如果把这两类内容混在一起，文件最后只会变成一团浆糊。一半太泛，一半太局部，而 Claude 每做一个任务都得把整包内容背在身上。

Anthropic 也建议把这些文件写得简洁一点。很好。很长的指令文件通常看起来就像有人试图把整本工程手册都塞进 prompt 里。这通常不会有好结果。

## 会话进行到一半时会出什么问题

规则有帮助，但救不了一个已经变乱的会话。

如果我花了二十条消息讨论某个子系统，然后突然切到另一个完全不同的问题，Claude 就可能还卡在前面的思路里。这很正常。我不会把长对话当成什么非得一直延续下去的神圣线程。
...
这也是我为什么喜欢用 markdown 文件来记计划和笔记。如果任务很大，我宁可把状态明确存下来，也不愿意赌一条超长线程能一直保持干净。
...
光是这些，对输出结果的影响就已经比大多数 prompt 技巧更大。

### 2. 添加项目级 `CLAUDE.md`

把命令、架构、命名和边界放进仓库里的文件。Anthropic 提供了 `/init` 来起草一版，这确实有用。但我之后还是会手动改，因为自动生成的指令只是草稿，不是成品。
...
很多关于编码代理“终极配置”的内容，很快就会滑向一种表演性的复杂化。
...
这套组合会让 Claude 更稳。随机冒出来的 fallback 更少，自作聪明的抽象更少，也更少出现那种十分钟后我才发现代理和我其实在解决两个略微错开的问题的情况。

如果你同时在用几个编码代理，别的产品上也会出现同样的模式。产品不同，结论一样：把基线设定一次讲清楚，别每天早上重新谈判。
