...
date: 2025-05-09
description: "القواعد التي أستخدمها في Cursor IDE لتحسين البرمجة بمساعدة الذكاء الاصطناعي، عبر أسلوب شفرة واضح، ومعالجة دقيقة للأخطاء، وسير عمل متسق بين المشاريع."
...
يدعم Cursor IDE ثلاث طبقات من القواعد:

1. قواعد `Rules for AI` داخل إعدادات Cursor IDE، وهي القواعد الأساسية التي تنطبق على جميع المشاريع
2. ملف `.cursor/index.mdc` مع نوع القاعدة `Always`، وهو مخصص لقواعد المشروع على مستوى المستودع ويحل محل نهج `.cursorrules` القديم
3. ملفات `.cursor/rules/*.mdc`، وهي قواعد ديناميكية لا تُفعَّل إلا عندما يعمل الذكاء الاصطناعي على مهام يطابقها وصف تلك القواعد

أشارك هنا القواعد الأساسية التي أضعها في إعداداتي العامة داخل Cursor IDE. هذه القواعد هي نقطة الانطلاق في كل أعمالي البرمجية. وعندما تجتمع مع قواعد المستودع والقواعد الديناميكية، تتكوّن منظومة تحافظ على جودة الشفرة وتبقي أسلوب العمل منسجمًا بين المشاريع.

> **تفضّل الشرح بالفيديو؟** أعددت شرحًا مصورًا كاملًا لهذا النظام. يمكنك مشاهدة [الشرح الكامل لقواعد Cursor IDE للذكاء الاصطناعي: المستويات الخمسة وملف .cursorrules (2025)](https://www.youtube.com/watch?v=gw8otRr2zpw) لترى هذه الأفكار مطبقة خطوة بخطوة.

[![إعداد قواعد Cursor IDE وتطبيقها عمليًا](/articles/cursor-ide-rules-tutorial.webp)](https://www.youtube.com/watch?v=gw8otRr2zpw)

## كيف تضبط قواعد Cursor لتحصل على أفضل أداء في البرمجة بمساعدة الذكاء الاصطناعي
...
- فضّل نماذج البيانات المنظمة على القواميس الفضفاضة مثل Pydantic أو الواجهات
...
- عندما تكون الاعتمادية مثبتة محليًا مثل `node_modules` أو `.venv`، اقرأ شفرتها المصدرية مباشرة حتى لو كانت ضمن الملفات المتجاهلة في git، فهذه أفضل طريقة لفهم كيفية عمل المكتبة
...
## كيف ترفع الكفاءة عبر استراتيجية متعددة المستويات لقواعد مشروع Cursor

عند العمل مع مزايا الذكاء الاصطناعي في Cursor IDE، وجدت أن تحسين القواعد عبر المستويات الثلاثة أمر حاسم. والفكرة الأساسية بسيطة: قلّل عدد التوكنات (`tokens`) التي تُرسل إلى نموذج اللغة في كل محادثة. فكلما قلّ عدد التوكنات المخصصة للسياق، زادت المساحة المتاحة لإنتاج استجابات أفضل.
...
### مسار تنفيذ قواعد مشروع Cursor في ثلاث خطوات
...
2. **انقل القواعد الخاصة بالمشروع إلى مستوى المستودع**  
   عندما ألاحظ أنماطًا تخص قاعدة شيفرة بعينها أو أرغب في مشاركة هذه التوجيهات مع زملائي، أنقلها إلى ملف `.cursor/index.mdc` مع نوع القاعدة `Always`. وهكذا أحصل على مرجعية مشتركة داخل الفريق مع الحفاظ على خفة إعداداتي العامة. وتجدر الإشارة إلى أن ملف `.cursorrules` القديم لا يزال يعمل، لكنه لم يعد النهج الموصى به.

3. **قسّم القواعد إلى ملفات مرتبطة بالسياق عند الحاجة**  
   إذا أصبحت قواعد المشروع على مستوى المستودع متضخمة، أقسمها إلى ملفات `.cursor/rules/*.mdc`. وهذا يقلّل استهلاك التوكنات، لأن القواعد ذات الصلة فقط هي التي تُفعَّل عند الحاجة. والنتيجة أن نموذج اللغة يمتلك حيّزًا أكبر للتركيز على المهمة الحالية بدلًا من الانشغال بإرشادات لا ترتبط بها.

هدفي بسيط: في كل محادثة مع المساعد الذكي، أزوّده بالقدر الكافي من السياق ليكون مفيدًا، من دون أن أستهلك قدرته في معلومات لا يحتاجها في تلك اللحظة.
...
### ملفات `.cursor/index.mdc` على مستوى المستودع: البنية وطريقة الاستخدام
...
هذا النهج يبقي الذكاء الاصطناعي مركزًا على ما يهم فعلًا، ويمنعه من الغرق في معلومات لا صلة لها بالمهمة الحالية.

### إدراج القواعد في منتصف الحوار: القيود وأفضل الممارسات

هناك قيد مهم ينبغي الانتباه إليه: تعمل قواعد `.mdc` المرتبطة بالسياق بأفضل صورة عندما تُطبَّق منذ بداية محادثة جديدة. فإذا كنت في منتصف حوار قائم داخل Cursor IDE ثم احتجت فجأة إلى قاعدة متخصصة، مثل إرشادات الاستعلام من قاعدة البيانات، فقد لا يصل الذكاء الاصطناعي تلقائيًا إلى ملف القاعدة المناسب. ويحدث ذلك لأن Cursor يكون قد ثبّت سياق المحادثة بالفعل، ولا يعيد دائمًا تقييم القواعد الواجب تطبيقها أثناء الحوار.
...
## كيف تطورت قواعد مشروع Cursor: من الإعدادات العامة إلى الأنظمة المرتبطة بالسياق
...
المفتاح هنا هو التوازن: إذا كانت القواعد قليلة جدًا فلن يفهم الذكاء الاصطناعي تفضيلاتك، وإذا كانت كثيرة جدًا فستهدر التوكنات على سياق لا حاجة له.
...
- اعتماد `styled components` للتنسيق
...
لكن المقياس الأهم بالنسبة إلي كان الحيّز الذهني. فعندما يتولى الذكاء الاصطناعي الاهتمام بمسائل الأسلوب، يستطيع المطور التركيز على حل المشكلة نفسها بدلًا من استنزاف انتباهه في قواعد التنسيق.
