...
title: "دفاتر Jupyter مع LLM داخل Cursor IDE: سير عمل أذكى لتحليل البيانات"
date: 2026-04-11
slug: "jupyter-notebooks-cursor-ide-llm-ai-ltahlil-albayanat"
description: "تعلّم كيف تستخدم دفاتر Jupyter داخل Cursor IDE مع LLM لتحليل البيانات، وبناء التصورات، وكتابة التوثيق، وتصدير تقارير احترافية من بيئة واحدة."
...
# من الإحباط إلى الانسياب: رحلتي مع دفاتر Jupyter وLLM داخل Cursor IDE

## المشكلة: نماذج اللغة الكبيرة ودفاتر Jupyter لا تنسجمان بسهولة

كانت الساعة الثانية صباحًا عندما اعترفت أخيرًا بالهزيمة. كان تحليل التسلسل الجيني الذي أعمل عليه متوقفًا لأن مساعدي المعتمد على نماذج اللغة الكبيرة داخل Cursor IDE لم يستطع فهم البنية المعقدة لملف دفتر Jupyter بصيغة JSON. في كل مرة طلبت فيها من الذكاء الاصطناعي المساعدة في شيفرة التصور البصري، كانت النتيجة ملف JSON مكسورًا لا يفتح أصلًا. جرّبت إرسال مقتطفات صغيرة بدلًا من الملف كاملًا، لكن ذلك جرّد الطلب من كل السياق المرتبط بخطوات المعالجة المسبقة. وفي الوقت نفسه كنت أتنقل بين ثلاث نوافذ: Jupyter في المتصفح، وVSCode للبرمجة "الحقيقية"، ومحرر آخر للتوثيق. الجمع بين قيود LLM وصيغة Jupyter وتبديل السياق المستمر جعل العمل الجاد على البيانات شبه مستحيل. وحتى مع مجموعات بيانات أبسط مثل Iris، كانت هذه الفجوة الأساسية تستنزف إنتاجيتي.

هل يبدو هذا مألوفًا؟ سير عمل علم البيانات مرهق أصلًا، ويزداد قسوة عندما تضطر إلى القفز باستمرار بين:
...
هنا اكتشفت حلًا غيّر طريقتي في العمل: استخدام دفاتر Jupyter مباشرة داخل Cursor IDE مع الاستفادة من قدرات الذكاء الاصطناعي. هذا الأسلوب يجمع بين:
...
إذا أردت فهمًا أعمق لما يتيحه دفتر Jupyter نفسه، فراجع [الوثائق الرسمية](https://jupyter-notebook.readthedocs.io/en/stable/notebook.html).
...
3. الشيفرة ومخرجاتها منفصلتان داخل أجزاء مختلفة من البنية
4. أي تعديل بسيط يتطلب فهم المخطط الكامل للملف حتى لا ينكسر
...
وأثناء بناء الدفتر، من المفيد الحفاظ على هذا النمط:
...
بمجرد طلب واحد فقط، تحصل على خلية مكتملة ومنسقة كما ينبغي لتحميل البيانات. لا حاجة إلى تذكر أسماء الدوال بدقة أو الصياغة المثالية؛ الذكاء الاصطناعي يتعامل مع ذلك.
...
## توجيه LLM باستخدام قواعد Cursor للحصول على شيفرة أفضل
...
1. نوع Setosa منفصل تقريبًا بالكامل في أي رسم يتضمن قياسات البتلات
...
وهذا نوع شائع من المشكلات في مشاريع علم البيانات: عدم توافق الإصدارات بين الحزم. لتشخيصه، أضفت خلية تتحقق من الإصدارات المثبتة:
...
أظهرت الخريطة الحرارية ارتباطًا قويًا جدًا، يصل إلى 0.96، بين طول البتلة وعرضها. هاتان الخاصيتان تتحركان معًا بوضوح في الطبيعة.
...
إذا كنت متعبًا من التنقل بين عدة أدوات لإنجاز عمل واحد في علم البيانات، فأنصحك بتجربة هذا النهج المتكامل. جهّز Jupyter داخل Cursor IDE، واستفد من المساعد الذكي، واختبر بنفسك كيف يبدو العمل عندما تجتمع الأدوات في مكان واحد. سيشكرك نفسك لاحقًا، خصوصًا في تلك الساعات المتأخرة من الليل.
