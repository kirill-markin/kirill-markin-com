...
keywords: [
  ...
  "أتمتة البريد باستخدام نماذج اللغة الكبيرة",
  "أتمتة Gmail عبر n8n",
  "نظام لتصنيف البريد الإلكتروني",
  "شرح سير عمل n8n",
  "تصفية البريد باستخدام GPT",
  "وسوم بريد إلكتروني تلقائية",
  "أتمتة إدارة البريد الإلكتروني"
]
title: "كيف تؤتمت تصنيف البريد الإلكتروني باستخدام n8n ونماذج اللغة الكبيرة"
date: 2026-04-11
description: "نظام مجرّب منذ ثلاثة أشهر لأتمتة تصنيف البريد الإلكتروني عبر n8n وGPT-5-nano، بحيث يقرر الذكاء الاصطناعي ما إذا كانت الرسالة تُؤرشف أو تُقرأ أو تحتاج إلى رد."
...
# كيف تؤتمت تصنيف البريد الإلكتروني باستخدام n8n ونماذج اللغة الكبيرة

سئمت من انتظار Gmail حتى يكتشف وحده أي الرسائل تستحق انتباهي فعلا. وبعد ثلاثة أشهر من ترك الذكاء الاصطناعي يدير تصنيف بريدي الشخصي، لم أعد أتصور العودة إلى الفرز اليدوي.

النظام بسيط إلى حد يبعث على الحرج: يضع GPT-5-nano كل رسالة في واحدة من ثلاث فئات فقط. إما أن تُؤرشف، أو تُترك للقراءة، أو تحتاج إلى رد. هذا كل ما في الأمر.

كل شيء يعمل عبر n8n، وتكلفته تكاد لا تُذكر لأنني أستخدم أرخص نموذج من OpenAI مع مخرجات منظَّمة. نعم، OpenAI يطّلع الآن على رسائلي الشخصية، لكن بصراحة من المحتمل أن محادثاتي في ChatGPT تكشف عني أكثر مما تكشفه هذه الرسائل أصلا.

هذه ليست فكرة نظرية أطرحها على سبيل التجربة. أنا أشغّل هذا النظام فعليا منذ ثلاثة أشهر، وهو يوفر عليّ ساعات كل أسبوع.

## لماذا بنيت نظام أتمتة البريد هذا

أتلقى عددا كبيرا جدا من الرسائل: نشرات بريدية، وإشعارات من الخدمات، ورسائل مهمة فعلا من أشخاص حقيقيين، وكل ذلك يتكدس في صندوق وارد واحد. وكان التصنيف المدمج في Gmail يفوّت أنماطا واضحة باستمرار، فيما كنت أقضي 20 دقيقة كل صباح فقط لأقرر ما الذي يستحق القراءة.

وجاءت لحظة الحسم حين أدركت أنني أؤرشف 80% من رسائلي من دون أن أقرأها أصلا. إذا كانت أغلب الرسائل لا تحتاج إلى انتباهي، فلماذا أبقى أنا من يقرر أيها يحتاجه؟

> **هل تفضل شرحا بالفيديو؟** أعددت عرضا عمليا خطوة بخطوة يشرح سير عمل أتمتة البريد بالكامل. [شاهد شرح أتمتة البريد باستخدام n8n وLLM](https://www.youtube.com/watch?v=xn1RyMlZudE) لترى عملية الإعداد كاملة أثناء التنفيذ.

[![عرض توضيحي لسير عمل أتمتة البريد باستخدام n8n وLLM](https://img.youtube.com/vi/xn1RyMlZudE/maxresdefault.jpg)](https://www.youtube.com/watch?v=xn1RyMlZudE)

## نظام الفئات الثلاث الذي يعمل فعلا

الذكاء الاصطناعي في هذا النظام يصنف كل رسالة في واحدة من ثلاث فئات لا غير:

- **الأرشفة**: النشرات البريدية التي اشتركت فيها ولا أقرأها، والإشعارات التلقائية، والرسائل الترويجية
- **للقراءة**: محتوى أريد الاطلاع عليه لكنه لا يحتاج إلى رد
- **للرد**: رسائل تحتاج إلى رد بشري مني

لا توجد بنية مجلدات معقدة، ولا مستويات أولوية متشابكة. فقط ثلاث قرارات بسيطة تغطي 100% من بريدي.
...
يعمل هذا المسار تلقائيا كلما وصلت رسالة جديدة إلى صندوق Gmail الخاص بي. يلتقط محتوى الرسالة، ويرسله إلى OpenAI لتصنيفها، ثم يضيف وسم Gmail المناسب ويؤرشف الرسائل التي لا تحتاج إلى انتباهي.

### العقد التي تحتاجها في n8n

ستحتاج إلى هذه العقد ضمن سير العمل:

1. **Gmail Trigger** - يراقب الرسائل الجديدة
2. **OpenAI Chat Model** - يصنف محتوى الرسالة
3. **Gmail** - يطبق الوسوم ويؤرشف الرسائل
4. **IF conditions** - يوجّه الرسائل بناء على قرار الذكاء الاصطناعي

الجزء الحاسم هنا هو إعداد عقدة OpenAI. هكذا ضبطتها:

![إعداد عقدة OpenAI LLM في n8n](/articles/assets/n8n-llm-node-config.webp)

## نص التعليمات الذي يجعل النظام يعمل

نص التعليمات المرسل إلى النموذج هو قلب هذا النظام. وبعد اختبار عشرات الصياغات المختلفة، كانت هذه الصيغة هي الأكثر ثباتا في النتائج:

> **ملاحظة**: هذا مجرد نص التعليمات الخاص بتصنيف البريد الإلكتروني. إذا كنت تريد سير عمل n8n الكامل، بما في ذلك جميع العقد والاتصالات، فانتقل إلى قسم "ملف JSON الكامل لسير عمل n8n" أدناه.

<details>
<summary>اضغط لعرض نص التعليمات الكامل للنموذج</summary>
...
أستخدم مخرجات منظَّمة حتى أضمن أن يعيد الذكاء الاصطناعي فئة صحيحة في كل مرة. لا أخطاء في تحليل الاستجابة، ولا حالات طرفية يقرر فيها النموذج أن يخترع تنسيقا غريبا للرد.
...
نعم، OpenAI يعالج الآن كل رسائلي الشخصية. هذا ليس مثاليا من ناحية الخصوصية، لكنني تقبلت ذلك لعدة أسباب.

أولا، أستخدم بالفعل مدير كلمات مرور مع رموز 2FA لا تعتمد على البريد الإلكتروني، لذلك فإن اختراق البريد وحده ليس كارثيا. ثانيا، OpenAI يعرف عني الكثير أصلا من استخدامي المعتاد لـChatGPT. ثالثا، توفير الوقت يستحق هذه المفاضلة بالنسبة إلى سير عملي الشخصي.
...
3. **الحصول على مفتاح OpenAI API** - أنشئ حسابا واحصل على مفتاح API الخاص بك
4. **استيراد سير العمل** - ستجد ملف JSON الكامل أدناه
5. **تخصيص نص التعليمات** - عدّل الفئات بحسب أنماط بريدك
...
وفوق ذلك، أستطيع تعديل المنطق في أي وقت. تريد إضافة فئة رابعة؟ عدّل نص التعليمات. تحتاج إلى معاملة مختلفة لرسائل مرسلين محددين؟ أضف عقدة شرطية. قواعد Gmail جامدة، أما هذا النظام فيتكيف مع ما أحتاجه.
...
7. جهّز رسالة البريد التي ستُرسل عند وقوع خطأ كخطة بديلة (fallback)
...
إذا كنت تفضل التعلم بصريا، فقد أعددت شرحا مفصلا بالفيديو يمر على العملية كاملة لإعداد هذا النظام:

[![شرح فيديو لأتمتة البريد باستخدام n8n وLLM](https://img.youtube.com/vi/xn1RyMlZudE/maxresdefault.jpg)](https://www.youtube.com/watch?v=xn1RyMlZudE)

يعرض الفيديو كل خطوة من إنشاء سير عمل n8n، وضبط تكامل OpenAI، وإعداد اتصالات Gmail، واختبار الأتمتة كاملة. سترى إعدادات العقد كما هي، ونص التعليمات المرسل إلى النموذج أثناء التشغيل، وكيف يصنف النظام رسائل حقيقية في الوقت الفعلي.
