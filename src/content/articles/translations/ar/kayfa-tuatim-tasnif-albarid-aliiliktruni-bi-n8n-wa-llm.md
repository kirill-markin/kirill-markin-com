...
title: "كيف تؤتمت فرز البريد الإلكتروني باستخدام n8n ونموذج لغوي كبير"
date: 2026-04-11
description: "هذا هو النظام الذي أعتمد عليه منذ ثلاثة أشهر لأتمتة تصنيف البريد عبر n8n وGPT-5-nano، ليحدد تلقائيا ما إذا كانت الرسالة تُؤرشف أو تُقرأ أو تحتاج إلى رد."
...
# كيف تؤتمت فرز البريد الإلكتروني باستخدام n8n ونموذج لغوي كبير

مللت انتظار Gmail حتى يكتشف بنفسه أي الرسائل تستحق انتباهي فعلا. وبعد ثلاثة أشهر من ترك الذكاء الاصطناعي يتولى تصنيف بريدي الشخصي، لم أعد أتصور العودة إلى الفرز اليدوي.

فكرتي بسيطة إلى حد محرج: يضع GPT-5-nano كل رسالة في واحدة من ثلاث خانات فقط: تُؤرشف، أو تُقرأ، أو تحتاج إلى رد. لا أكثر.

النظام كله يعمل على n8n، وتكلفته شبه معدومة لأنني أستخدم أرخص نموذج لدى OpenAI مع مخرجات منظَّمة. نعم، هذا يعني أن OpenAI يرى رسائلي الشخصية كلها، لكن بصراحة من المرجح أن محادثاتي في ChatGPT تكشف عني أكثر مما تكشفه هذه الرسائل أصلا.

هذه ليست فكرة نظرية أطرحها فحسب. أنا أشغّل هذا النظام فعليا منذ ثلاثة أشهر، وهو يوفر علي ساعات كل أسبوع.

## لماذا بنيت نظام أتمتة البريد هذا

أتلقى رسائل كثيرة جدا: نشرات بريدية، وإشعارات من الخدمات، ورسائل مهمة فعلا من أشخاص حقيقيين، وكل ذلك يتكدس في صندوق وارد واحد. وكان التصنيف المدمج في Gmail يفوّت أنماطا واضحة باستمرار، بينما كنت أقضي 20 دقيقة كل صباح فقط لأقرر ما الذي يستحق القراءة.

وجاءت لحظة الحسم عندما أدركت أنني أؤرشف 80% من رسائلي من دون أن أقرأها أصلا. فإذا كانت أغلب الرسائل لا تحتاج إلى انتباهي، فلماذا أكون أنا من يقرر كل مرة أي الرسائل تستحقه؟

> **هل تفضّل شرحا بالفيديو؟** أعددت عرضا عمليا خطوة بخطوة يشرح سير عمل أتمتة البريد بالكامل. [شاهد شرح أتمتة البريد باستخدام n8n ونموذج لغوي كبير](https://www.youtube.com/watch?v=xn1RyMlZudE) لترى عملية الإعداد كاملة أثناء التنفيذ.
...
## نظام الفئات الثلاث الذي ينجح فعلا

الذكاء الاصطناعي في هذا النظام يصنّف كل رسالة ضمن واحد من ثلاثة خيارات لا غير:

- **الأرشفة**: نشرات بريدية اشتركت فيها ولا أقرأها، وإشعارات آلية، ورسائل ترويجية
- **القراءة**: محتوى أريد الاطلاع عليه، لكنه لا يحتاج إلى رد
- **الرد**: رسائل تحتاج إلى رد بشري مني

لا توجد بنية مجلدات معقدة، ولا مستويات أولوية متشابكة. مجرد ثلاثة قرارات بسيطة تغطي 100% من بريدي الإلكتروني.

## إعداد سير عمل n8n

إليك سير عمل n8n الكامل الذي يتكفل بكل شيء تلقائيا:
...
الجزء الأهم هنا هو إعداد عقدة OpenAI. هذا هو الضبط الذي أستخدمه:

![إعداد عقدة OpenAI للنموذج اللغوي في n8n](/articles/assets/n8n-llm-node-config.webp)

## الموجّه الذي يجعل النظام يعمل

الموجّه (Prompt) المرسل إلى النموذج هو قلب هذا النظام. وبعد اختبار عشرات الصياغات المختلفة، كانت هذه الصيغة هي الأكثر ثباتا في النتائج:

> **ملاحظة**: هذا مجرد الموجّه الخاص بتصنيف البريد الإلكتروني. إذا كنت تريد سير عمل n8n الكامل، بما في ذلك جميع العُقد والوصلات، فانتقل إلى قسم "ملف JSON الكامل لسير عمل n8n" أدناه.

<details>
<summary>اضغط لعرض الموجّه الكامل للنموذج</summary>
...
أستخدم مخرجات منظَّمة لكي أضمن أن يعيد الذكاء الاصطناعي فئة صحيحة في كل مرة. لا أخطاء في تحليل الاستجابة، ولا حالات شاذة يقرر فيها النموذج أن يبتكر تنسيقا غريبا للرد.
...
## اعتبارات الأمان والخصوصية ولماذا أتقبل هذا الخيار

نعم، يعالج OpenAI الآن كل رسائلي الشخصية. هذا ليس مثاليا من ناحية الخصوصية، لكنني متصالح معه لعدة أسباب.
...
2. **ربط Gmail** - ستحتاج إلى ربط حساب Gmail الخاص بك وتوثيقه
3. **الوصول إلى OpenAI API** - أنشئ حسابا واحصل على مفتاح API الخاص بك
4. **استيراد سير العمل** - استخدم ملف JSON المصدَّر أو النسخة الكاملة الواردة أدناه
5. **تخصيص الموجّه** - عدّل الفئات بما يناسب أنماط بريدك
...
## لماذا يتفوّق هذا على مزايا Gmail المدمجة

التصنيف التلقائي في Gmail مصمم للجميع، وهذا يعني عمليا أنه لا يناسب أحدا تماما. أما نظامي فهو يتعلم نمط بريدي وتفضيلاتي أنا.

وفوق ذلك، يمكنني تعديل المنطق في أي وقت. تريد إضافة فئة رابعة؟ عدّل الموجّه. تحتاج إلى معاملة مختلفة لرسائل مرسلين محددين؟ أضف عقدة شرطية. قواعد Gmail جامدة، أما هذا النظام فيتكيف مع ما أحتاجه.
...
الإعداد مباشر، والتكلفة المستمرة منخفضة جدا، وتوفير الوقت حقيقي. إذا كنت غارقا في البريد كما كنت أنا، فقد يكون هذا النهج جديرا بالتجربة.

تذكّر فقط: ابدأ ببساطة، اختبر جيدا، ولا تؤتمت أي شيء لا يمكنك التراجع عنه بسهولة.
