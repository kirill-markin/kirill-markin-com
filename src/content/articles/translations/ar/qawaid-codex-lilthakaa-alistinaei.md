...
description: "كيف أستخدم التعليمات المخصصة في Codex وملف AGENTS.md وتطبيق Mac للحفاظ على أسلوب برمجي ثابت عبر المشاريع المختلفة، من دون الحاجة إلى إعادة شرح قواعدي في كل مرة."
...
لم أستخدم Codex بجدية إلا منذ نحو شهر، وبالدرجة الأولى بعدما جعل GPT-5.4 التجربة أكثر قابلية للاعتماد بالنسبة إلي. لذلك فهذه ليست مقالة من نوع "خمس سنوات من الخبرة التي اختبرتها المعارك". الملاحظة هنا أبسط بكثير: بدأ Codex ينجح معي في اللحظة التي توقفت فيها عن التعامل معه كأنه مجرد واجهة أنيقة أخرى لكتابة المطالبات، وبدأت بدلًا من ذلك أعطيه تعليمات ثابتة من البداية.

توفّر OpenAI كلًا من Codex CLI وتطبيق Mac. وبالنسبة إلي، كلاهما يقوم على الفكرة نفسها: تعليمات دائمة، وملف `AGENTS.md`، وقواعد خاصة بالمستودع، ووكيل يبدأ العمل وهو يعرف سلفًا أسلوب التطوير الذي أفضله. أفضل تطبيق Mac لأنه ببساطة أمتع وأكثر راحة في الاستخدام اليومي من مجرد نافذة طرفية إضافية.

Codex CLI يؤدي هذا الدور جيدًا أصلًا. أما تطبيق Mac فيمنحني واجهة أجمل وأكثر راحة حول سير العمل نفسه. وما زالت القواعد التي تهمني هي نفسها: تعريف صارم للأنواع، وتعديلات محدودة، وأخطاء صريحة، ومن دون حلول احتياطية عشوائية، و`docstrings` داخل الشفرة بدل الشروح المبعثرة. لا أريد أن أعلّم Codex هذه الأمور من الصفر في كل مهمة جديدة. أريد أن يكون هذا الأساس حاضرًا من البداية.

عمليًا، مكان ذلك هو `Settings -> Personalization -> Custom instructions`.

وخلف الكواليس، ترتبط تعليمات التطبيق هذه بملف `AGENTS.md` الشخصي. وهذا ممتاز. أحصل على تجربة تطبيق ألطف، من دون أن أخسر الوضوح الذي يمنحه لي وجود ملف حقيقي على طريقة CLI.
...
وهذا بالضبط هو النموذج الذي أريده. أستطيع استخدام التطبيق كواجهة أساسية من دون أن أفقد وضوح الملف الموجود خلفه.
...
- يجب أن تكون رسائل الخطأ واضحة وتشير إلى الإجراء المطلوب
...
- يجب أن يستخدم التسجيل حقولًا مهيكلة بدل حشر القيم الديناميكية داخل نص الرسالة
...
يدعم Codex تدرّج ملفات `AGENTS.md` بشكل جيد. توثّق OpenAI ملفًا عامًا في `~/.codex/AGENTS.md`، ثم ملفات خاصة بالمستودع وملفات داخلية أكثر تحديدًا كلما أصبح مسار العمل أكثر تحديدًا.
...
وليس السبب أن CLI ضعيف. على العكس، CLI جيد جدًا أصلًا. لكن التطبيق ألطف بكثير في الاستخدام اليومي. Codex نفسه يعمل في الخلفية، لكن الواجهة أكثر راحة بكثير.
...
## يبقى AGENTS.md الخاص بالمشروع مهمًا، لكنه ليس بطل الرواية
...
- الملفات الأعمق في المسارات الفرعية مخصصة للحالات النادرة التي تحتاجها فعلًا
...
أنا لا أريد تصورًا منفصلًا تمامًا عن CLI. أريد ملف `AGENTS.md` الشخصي نفسه، وإرشادات المستودع نفسها، والضوابط نفسها في الحالتين.
