---
title: "لماذا استبدلت 17 أداة للوكلاء بلغة استعلام مخصّصة تشبه SQL"
date: 2026-04-11
slug: "istabdalat-17-adat-lilwukala-bi-dsl-shabih-bisql"
description: "كيف اختزلت 17 أداة للوكلاء في Flashcards Open Source App إلى واجهة واحدة بلغة استعلام مخصّصة تشبه SQL، بحيث تتمكن النماذج الداخلية والخارجية من تعلّمها بسرعة."
tags: [productivity, ai, llm]
publish: true
thumbnailUrl: "/articles/sql-like-dsl-for-ai-agents.webp"
keywords: [
  "لغة استعلام مخصّصة تشبه SQL للوكلاء",
  "تصميم API لوكلاء الذكاء الاصطناعي",
  "استدعاء أدوات نماذج LLM",
  "واجهة موحّدة للوكلاء",
  "واجهة واحدة للوكلاء",
  "دمج أدوات الوكلاء",
  "Flashcards Open Source App",
  "لغة استعلام لوكلاء الذكاء الاصطناعي",
  "تصميم واجهات مناسب لنماذج LLM",
...
# لماذا استبدلت 17 أداة للوكلاء بلغة استعلام مخصّصة تشبه SQL

يوم الثلاثاء الماضي كنت أراجع وثائق طبقة الوكلاء في Flashcards Open Source App، ووصلت إلى تلك اللحظة التي يعرفها أي مهندس أنظمة خلفية: كل شيء مرتب، مضبوط بالأنواع، واضح... وصعب الاحتمال قليلًا.
...
لكن الواجهة كانت مليئة بذلك النوع من الضجيج الذي تتكدس به واجهات LLM عادة. المهندس البشري يستطيع أن يمر على الوثائق مرة واحدة، ويبني عميلًا، ثم يكمل طريقه. أما النموذج فلا يملك هذا الترف. هو مضطر إلى إعادة تعلّم الواجهة نفسها من الأمثلة، والوصف، ورسائل الخطأ، مرة بعد أخرى. وإذا وزعت نية بسيطة على عدد كبير جدًا من الأدوات، فإن النموذج يدفع ثمن ذلك في كل مرة.

هذه هي طبقة الوكلاء التي تقف خلف [flashcards-open-source-app.com](https://flashcards-open-source-app.com/)، لذلك كان يهمني أن تبقى الواجهة الظاهرة سهلة التعلّم، لا أن تكون صحيحة تقنيًا فقط.

لذلك ضغطت كل هذا في واجهة واحدة تستخدم لغة وصف نطاقي (DSL) تشبه SQL.

وليست PostgreSQL خامًا. لست متهورًا إلى هذه الدرجة.

![واجهة بلغة استعلام مخصّصة تشبه SQL تستبدل 17 أداة منفصلة للوكلاء](/articles/sql-like-dsl-for-ai-agents.webp)
...
من زاوية الأنظمة الخلفية يبدو هذا مرتبًا. كل أداة تنفذ مهمة واحدة. كل مخطط واضح. وواجهة OpenAPI تبدو محترمة. هذا تصرّف كلاسيكي من مهندس أنظمة خلفية.

أما من زاوية الوكيل، فهذه مجرد إجراءات ورقية.
...
العقد البرمجي العام الجديد اختُزل في أداة واحدة:
...
وهذا أقرب بكثير إلى الطريقة التي تختبر بها نماذج LLM الأنظمة فعلًا: تجرّب شيئًا، وتقرأ الخطأ، ثم تعيد المحاولة. ووجود لغة واحدة تشبه SQL يخدم هذه الحلقة أفضل بكثير من 17 أداة منفصلة.
...
إذا اخترعت لغة مخصّصة بصيغة JSON، فعلى النموذج أن يتعلم أفعالي أنا، وبنيتي المتداخلة، وآليات التصفية، والحالات الطرفية، وحتى المزاج الذي كنت فيه عندما سميت الأشياء في ذلك الأسبوع. أما إذا أعطيته صيغة تشبه SQL، فعادة يصل قريبًا من الإجابة الصحيحة من المحاولة الأولى.
...
بل تحلل السلسلة الشبيهة بـ SQL، وتتحقق منها وفق القواعد المنشورة، ثم تترجمها إلى العمليات الداخلية نفسها التي يستخدمها المنتج أصلًا. وسلسلة SQL هنا هي اللغة العامة التي أكشفها للخارج، لا نفقًا مباشرًا إلى طبقة التخزين.
...
وبدلًا من نشر موارد إضافية مثل `tags_summary` أو `due_cards` أو صيغ جاهزة أخرى، أضفت بعض القوة إلى لغة الاستعلام نفسها. وأهم ما أضفته كان `GROUP BY` وبعض دوال التجميع.
...
ومع ذلك اخترت `OFFSET` لهذه الواجهة لأنه أسهل لمن يكتبون الوكلاء، وأسهل لعرضه في الأمثلة، وأسهل على النموذج أن يولده من دون معرفة إضافية بالبروتوكول. في هذه الواجهة البرمجية، أنا أهتم أكثر بسهولة الاستخدام من أول محاولة، لا بسلوك مثالي لتقسيم الصفحات مع بيانات متحركة.
...
لن أستخدم هذا النهج إذا كان المنتج يعتمد كثيرًا على أفعال مجال معقدة لا تأخذ شكل CRUD التقليدي.

إذا كان الفعل الحقيقي شيئًا مثل `submit_review` أو `run_scheduler` أو `merge_learning_state`، فإن التظاهر بأن كل شيء مجرد `UPDATE` يجعل الواجهة البرمجية أسوأ عادة. في هذه الحالات سأبقي الأوامر الصريحة للعمليات المعقدة، وأستخدم اللغة الشبيهة بـ SQL لطبقة القراءة العامة، وعمليات CRUD الأساسية، والتقارير الخفيفة.
...
استبدلت فهرسًا واسعًا من الأدوات بلغة استعلام واحدة تعرف معظم نماذج LLM شيئًا منها أصلًا.
